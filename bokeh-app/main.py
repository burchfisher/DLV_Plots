...
# For the sample group-timeline plot
plot = figure(tools=TOOLS, plot_width=1800, plot_height=ph, title=None, tooltips=ttips, x_axis_type='datetime', y_range=df.Type.unique())
plot.scatter(x='Date', y=jitter('Type', width=0.6, range=plot.y_range), source=source, legend_field='Type', fill_alpha=fa, size=ms, color=colors, muted_color=colors, muted_alpha=0.2)
plot.xaxis.axis_label = 'Date'
plot.yaxis.axis_label = 'Group'
plot.legend.location = (875,10)
pl.append(plot) 
...
# a5a = row(pl[16],pl[17])
# a5b = row(pl[18])
a5 = row(pl[16],pl[17],pl[18],pl[19])
a6 = row(pl[20])
